function [ importance_map ] = My_obj_sal_map(im_name,im_sal, SHOW_MAP)
...
im =  imread(im_name);
mapbig = double(imread(im_sal));
%disp(max(mapbig(:)))
%mapbig = double(imread('DSCF0471_ASNet_impor.png'));
mapbig = (mapbig-min(mapbig(:)))/(max(mapbig(:))-min(mapbig(:)));
%mapbig = mapbig.master_map_resized;
