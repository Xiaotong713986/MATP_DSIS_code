function [ Vertex_updated, Vertex_updatedR ] = SNS_Opt_Iter_M3(Vertex_set_org ,Vertex_warped, Vertex_warpedR, importance_quad, importance_quadR, alpha_matrix, alpha_matrixR)
            ...
            for tt=1:4
                ...
                if temp 
                    ...
                    if flag == 1
                        ...
                        f_loc_R_index = find(f_loc_R_br==min(f_loc_R_br));
                        f_loc_R_br = f_loc_R_br(f_loc_R_index);
                        Q_hR = mod(f_loc_R_br-+ Quad_w*Quad_h,Quad_h);
                        if Q_hR ==0
                            Q_hR = h_V;
                        end
                        Q_hR = min(h_V-1,Q_hR);
                        Q_wR = min(round((f_loc_R_br-Quad_w*Quad_h-Q_hR)/Quad_h+1),w_V-1);
...
%                     - Vertex_set_org(Q_h-1,Q_w,Layer_Vertex) - Vertex_set_org(Q_h,Q_w-1,Layer_Vertex));
                ...
                %if(ALIGIN_TERM && f_logic)
                    ...
                    if print_flag
                        disp('###########  top left   #############')
                        disp(['[h_V w_V]:  (' ,num2str(h_V),', ',num2str(w_V),')'])
