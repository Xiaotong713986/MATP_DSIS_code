function [new_imgL,new_imgR,new_salL, new_salR] =cutImg2(imgL, imgR, salL, salR, wRatio, hRatio)
    [h,w,~] = size(imgL);
    hRatio=1;
    left = 1;
    right = w;
...
%        if salL_v>threshold*max(sal_value) || salR_v>threshold*max(sal_value)
