function [ Vertex_updated, Vertex_updatedR ] = SNS_Opt_Iter_M2(Vertex_set_org ,Vertex_warped, Vertex_warpedR, importance_quad, importance_quadR, alpha_matrix, alpha_matrixR)
...
Depth_TERM = 0;
Lambda_term = 0.01;
ALIGIN_AXIS = 2; %% 1 for y axis,2 for x axis
...
%                         Q_hR = mod(alpha_matrixR(temp,5),Quad_h);
%                         if Q_hR ==0
%                             Q_hR = h_V;
%                         end
...
%             end
            ...
            if( (Q_h - 1) > 0 && (Q_w - 1) > 0)
                ...
                f_index = find(alpha_matrix(:,5)==Vector_loc-1-h_V);  
                f_loc_R = alpha_matrixR(f_index,5)+ h_V*w_V;
                center_loc_R = f_loc_R+1+h_V;
                Q_hR = mod(center_loc_R-w_V*h_V,h_V);
                if Q_hR ==0
                    Q_hR = h_V;
                end
                Q_wR = round((center_loc_R-w_V*h_V-Q_hR)/h_V+1);
...
%                     - Vertex_set_org(Q_h-1,Q_w,Layer_Vertex) - Vertex_set_org(Q_h,Q_w-1,Layer_Vertex));
                ...
                %if(ALIGIN_TERM && f_logic)
                    ...
                    disp(['center_loc_R:  ' ,num2str(center_loc_R(1)),',  f_index:  ',num2str(f_index(1))])
