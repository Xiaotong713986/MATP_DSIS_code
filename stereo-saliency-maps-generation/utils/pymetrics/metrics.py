...
def SIM(saliency_map1, saliency_map2):
    ...
    # saliency_map1 = normalize_map(saliency_map1)
    # saliency_map2 = normalize_map(saliency_map2)
    # saliency_map1 = saliency_map1/(np.sum(saliency_map1)*1.0)
    # saliency_map2 = saliency_map2/(np.sum(saliency_map2)*1.0)
    # x,y = np.where(saliency_map2>0)
    # sim = 0.0
    # for i in zip(x,y):
    #     sim = sim + min(saliency_map2[i[0],i[1]],saliency_map1[i[0],i[1]])
    # return sim
...
def KLdiv(saliencyMap, fixationMap):
    ...
    #convert to float
    map1 = saliencyMap.astype(np.float32)
    map2 = fixationMap.astype(np.float32)
