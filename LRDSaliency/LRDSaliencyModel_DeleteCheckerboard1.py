...
class Model:
    ...
    def LRDSaliency_inference(left, right, is_training):
        with tf.variable_scope("flownet", reuse=tf.AUTO_REUSE):
            ...
            wh = conv6_1.shape[1:3]
            print('The shape of feature map:{}'%(wh))
            deconv1_a =  tf.image.resize_images(conv6_1,[wh[0]*4,wh[0]*4],method=0)
            deconv1_a = tf.layers.conv2d(deconv1_a, 512, 3, 1, 'same', name='deconv1_a', kernel_initializer=tf.contrib.layers.variance_scaling_initializer())
			...
			#[? ,8,8,512]
            wh = deconv1_a.shape[1:3]
            deconv2_a = tf.image.resize_images(deconv1_a,[wh[0]*4,wh[0]*4],method=0)
            deconv2_a = tf.layers.conv2d(deconv2_a, 128, 5, 1, 'valid', name='deconv2_a', kernel_initializer=tf.contrib.layers.variance_scaling_initializer())
            ...
            wh = deconv2_a.shape[1:3]
            deconv3_a = tf.image.resize_images(deconv2_a,[wh[0]*2,wh[0]*2],method=0)
            deconv3_a = tf.layers.conv2d(deconv3_a, 32, 3, 1, 'same',name = 'deconv_3_a', kernel_initializer=tf.contrib.layers.variance_scaling_initializer())
            ...
            wh = deconv3_a.shape[1:3]
            predict = tf.image.resize_images(deconv3_a,[wh[0]*2,wh[0]*2],method=0)
            predict = tf.layers.conv2d(predict, 1, 3, 1, 'same',name = 'predict', kernel_initializer=tf.contrib.layers.variance_scaling_initializer())
