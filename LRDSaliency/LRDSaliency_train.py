...
def get_arguments():
    ...
    parser.add_argument('--batch',type=int,default=1, help='length of video')
...
all_path = './results_sis3d'
SaveFile_name = 'save_model_sis3d'
...
def main():
    ...
    postfix_str="figure_model_SiS3D"
    ...
    with tf.Session() as sess:
        ...
        while _step < max_iter:
            ...
            if _step % 200 ==0:
                saver.save(sess, SaveFile + '/' + 'train_IDSENet_SiS3D_',global_step = _step)
            ...
            if _step % validation_iter==0:
                ...
                while data_tuple is not None:
                    ...
                    if len(valid_density_data)<1:
                        break
                    #print("length: left: {},right: {},density:{}".format(len(valid_frame_data_left), len(valid_frame_data_right),len(valid_density_data )))
                    pred_valid = sess.run(pred, feed_dict ={inputs_left: valid_frame_data_left, inputs_right: valid_frame_data_right})
                    # updated.
