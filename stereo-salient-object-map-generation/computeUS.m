...
imwrite(UniSal,'unisal_test.jpg');
...
imwrite(Salmask*255,'salMask.jpg');

valLAB = [superpixels{left}.Lab;superpixels{right}.Lab];
colDistM = squareform(pdist(valLAB));
%cold_test = (colDistM-min(colDistM(:)))/(max(colDistM(:))-min(colDistM(:)));
%imwrite(cold_test,'colorDist.jpg');
...
conp = 255*(ConSPix-min(ConSPix(:)))/(max(ConSPix(:))-min(ConSPix(:)));        
imwrite(uint8(conp),'conpixel.jpg');
